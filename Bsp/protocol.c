...
cmd_chassis_speed test_vel_info;

/*************************** 对数据打包 ****************************/

int16_t ProtocolPack(uint8_t* data,
                     uint16_t len,
                     uint8_t cmd_id,
                     uint8_t* pack) {
...
void DataHandle(uint8_t* protocol_packet) {
    ...
    uint16_t data_length =
        p_header->pack_len - HEADER_LEN - CMD_LEN - CRC_DATA_LEN;
    uint8_t cmd_id = *(uint8_t*)(protocol_packet + HEADER_LEN);
    ...
    switch (cmd_id) {
        ...
        case CMD_SET_CHASSIS_SPEED: {
            test_vel_info.v = *(uint16_t*)(data_addr);
            test_vel_info.w = *(uint16_t*)(data_addr + 2);
            CmdDecomposition(((double)(test_vel_info.v)) / 1000.0,
                             ((double)(test_vel_info.w)) / 1000.0);
        } break;
        default:
            break;
    }
